...
public:
    int smallestDistancePair(vector<int>& nums, int k) {
        ...
        while(left<=right){     // 1. 先对距离区间[left,right]进行二分
            int mid=(left+right)/2,cnt=0;   // 当left<right时，取不到right；当left==right时，取到right
            // 计算所有距离小于等于mid的数组
...
public:
    int subarraySum(vector<int>& nums, int k) {
        int pre=0,ans=0;        // pre记录：到当前元素为止，所有元素之和
        unordered_map<int,int>map;      // map中：key为前缀和；value为满足前缀和等于key的下标个数
        map[0]=1;
        for(int i=0;i<nums.size();i++){
            pre+=nums[i];
