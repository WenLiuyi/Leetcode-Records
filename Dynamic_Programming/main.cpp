...
// dp[i][j]: 以下标i-1结尾的word1，以下标j-1结尾的word2, 最近编辑距离
// 状态转移：
// 1. word1[i-1]==word2[j-1]: 不操作, dp[i][j]=dp[i-1][j-1].
// 2. word1[i-1]!=word2[j-1]:
// 2.1 word1删除元素：dp[i][j]=dp[i-1][j]+1
// 2.2 word1添加元素：dp[i][j]=dp[i][j-1]+1
// 2.3 word1替换元素：dp[i][j]=dp[i-1][j-1]+1
